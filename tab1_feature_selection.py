...
# Специфічні імпорти для Завдання 1
...
# --- Допоміжна функція (тільки для цього файлу) ---
...
def create_energy_dataset(n_samples=1000):
    ...
    # Інформативні ознаки (5) - Використовуємо ключі зі словника
    ...
    # Шумові ознаки (5) - Використовуємо ключі зі словника
...
def create_tab(tab_control):
    ...
    # --- ВИПРАВЛЕНО: Зберігаємо оригінальні англійські назви для коду ---
    FEATURE_NAMES_ENG = list(df.columns.drop('system_load'))
    # Створюємо список українських назв для відображення
    FEATURE_NAMES_UKR = [feature_name_mapping.get(name, name) for name in FEATURE_NAMES_ENG]
    TARGET_NAME = 'system_load'

    X = df[FEATURE_NAMES_ENG] # Використовуємо англійські для даних
    y = df[TARGET_NAME]
    ...
    def add_result_row(frame, label_text, value_text, selected_indices_text):
        ...
        # --- ВИПРАВЛЕНО: Виводимо список українських назв ---
        ttk.Label(frame, text=selected_indices_text, font=("Courier", 11)).grid(row=frame.grid_size()[1]-1, column=2, sticky="w", padx=20)

    # Функція для перетворення індексів у УКРАЇНСЬКІ назви
    def get_feature_names_ukr(indices):
        # Беремо англійську назву за індексом, потім перекладаємо
    ...
    # --- ВИПРАВЛЕНО: Легенда українською ---
    ...
    ax1.set_ylim(min(r2_scores) * 0.95 if min(r2_scores) > 0 else min(r2_scores)*1.05,
                 max(r2_scores) * 1.05 if max(r2_scores)>0 else max(r2_scores)*0.95) # Коректний ylim
