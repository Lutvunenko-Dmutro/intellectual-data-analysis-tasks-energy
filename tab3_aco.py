...
def calculate_path_distance_from_list(path_list, distances_matrix):
    ...
    for i in range(num_cities):
        dist += distances_matrix[path_list[i]][path_list[(i + 1) % num_cities]] 
    return dist

def two_opt_optimize(initial_path_aco_format, distances_matrix, max_iterations=10000): 
    """
    ...
    while improved and iterations < max_iterations:
        improved = False
        iterations += 1
        for i in range(len(best_path_list) - 1): 
            for k in range(i + 1, len(best_path_list)):
                ...
                if new_dist < current_dist:
                    best_path_list = two_opt_swap(best_path_list, (i + 1) % n, k) 
                    best_distance = calculate_path_distance_from_list(best_path_list, distances_matrix) 
                    improved = True
                    break 
            if improved:
